...
encryptionkey = pd.read_csv('decodekeynew.csv',  sep=',', names=['Character', 'Byte'], header=None, skiprows=[0])
...
def GUI():
    ...
    b = tk.Radiobutton(root, text='encrypt message', variable=v, value='e')
    canvas1.create_window(150, 45, window=b)

    b2 = tk.Radiobutton(root, text='decrypt message', variable=v, value='d')
